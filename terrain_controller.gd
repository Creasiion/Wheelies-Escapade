...
@export_dir var terrian_blocks_path = "res://Terrain Generation"
...
func _physics_process(delta: float) -> void:
	...
	if player:
		# Smoothly interpolate terrain speed toward player's speed
		terrain_velocity = lerp(terrain_velocity, player.move_speed, delta * 5.0)
...
func _progress_terrain(delta: float) -> void:
	...
	if terrain_belt[0].global_position.z > $DespawnMarker.global_position.z:
